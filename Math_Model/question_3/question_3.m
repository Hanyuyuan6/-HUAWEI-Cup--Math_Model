...
total_time = 100; % 总时间（秒）- 只使用前100秒的数据

% 初始化调度指令数据和风速等参数的矩阵
power_schedule = zeros(total_time, num_turbines); % 调度指令数据 (100×100)
wind_speed = zeros(total_time, num_turbines); % 风速矩阵 (100×100)
pitch_angle = zeros(total_time, num_turbines); % 桨距角矩阵 (100×100)
omega_r = zeros(total_time, num_turbines); % 转速矩阵 (100×100)
power_out = zeros(total_time, num_turbines); % 实际输出功率矩阵

% 遍历每台风机，提取相应的数据（只取前100秒）
...
for t = 1:total_time
    ...
    obj_fun = @(P) weightedSumFatigueDamageWithPenalty(P, V_t, pitch_t, omega_t, P_out_t, ...
...
variance_opt = variance_opt / 1e12; % 转换为MW²
disp(variance_opt);
variance_avg = var(power_reference_avg, 0, 2); % 平均分配时功率分配的方差
variance_avg = variance_avg / 1e12; % 转换为MW²
disp(variance_avg);
% 绘制功率分配方差对比图
figure('Name', '优化前后功率分配方差对比', 'NumberTitle', 'off');
plot(1:total_time, variance_opt, 'g-', 'LineWidth', 1.5); hold on; % 优化后方差 
...
function weighted_damage = weightedSumFatigueDamageWithPenalty(P, V, pitch, omega_r, P_out, ...
    ...
    for turbine = 1:num_turbines
        % 主轴扭矩雨流计数
        [cycles_shaft, amplitudes_shaft, means_shaft, ~] = rainflowCounting(T_shaft(turbine));
        % Goodman 修正
        L_shaft = applyGoodmanCorrection(amplitudes_shaft, means_shaft, sigma_b);
        % 疲劳寿命
        Nfi_shaft = C_shaft ./ (L_shaft .^ m_shaft);
        ...
        % 塔架推力雨流计数
        [cycles_tower, amplitudes_tower, means_tower, ~] = rainflowCounting(F_tower(turbine));
        % Goodman 修正
        L_tower = applyGoodmanCorrection(amplitudes_tower, means_tower, sigma_b);
        % 疲劳寿命
        Nfi_tower = C_tower ./ (L_tower .^ m_tower);
...
function [T_shaft, F_tower] = estimateLoads(P, V, pitch, omega_r, P_out, ...
    ...
    % 检查 if columns are consistent
    if size(lambda,1) ~= size(pitch,1) || size(lambda,1) ~= size(P,1) || size(lambda,1) ~= size(power_diff,1)
        error('Columns of lambda, pitch, P, power_diff must have the same number of rows.');
    end
